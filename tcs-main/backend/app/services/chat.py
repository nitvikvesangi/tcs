...
def _llm_response(messages: List[Dict], store_id: Optional[int]) -> Dict[str, Any]:
    """Real LLM integration via ai_engine (NVIDIA NIM / Groq fallback)."""
    import sys, os
    # Add project root to path so ai_engine can be imported
    ...
    try:
        ...
        system_prompt = (
            "You are an intelligent retail AI assistant for a Quick Commerce dark-store network. "
            "You help store managers make data-driven decisions about promotions, inventory, and sales. "
            "Be concise, professional, and data-focused. Use ₹ for Indian Rupee amounts. "
            "Format your answers in markdown for readability."
        )
        llm_messages = [{"role": "system", "content": system_prompt}] + messages

        response_text = _call_llm(llm_messages, temperature=0.4, max_tokens=600)
        return {
            "response": response_text,
            "tool_calls_made": [],
            "demo_mode": False,
        ...
        return {
            "response": f"⚠️ AI response unavailable ({e}). Running in demo mode.\n\nTry asking: 'Which products are expiring?', 'Show me alerts', or 'List stores'.",
            "tool_calls_made": [],
            "demo_mode": True,
        }
...
def chat(
    ...
    has_api_key = bool(
        ...
        or os.getenv("NVIDIA_API_KEY") or os.getenv("GROQ_API_KEY")
    )
