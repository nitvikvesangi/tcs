...
def _build_item(r: dict) -> dict:
    """Convert one CSV row dict into the shape React expects."""
    ...
    # Use ML model prediction if available, else fall back to CSV ground truth
    if _predict_action:
        try:
            pred = _predict_action(r)
            action = pred["action"]
    ...
    return {
        ...
        "explanation": f"{pname} ({pid}) recommended for {action} at {disc_pct:.0f}% discount. "
                       f"Stock: {current_stock} units, Expiry: {days_exp} days, "
        ...
        "reasons": [
            ...
            f"Demand trend: {r.get('demand_trend_pct', 0.0)}%",
            f"Stockout risk: {stockout_risk:.1f}%",
        ],
        "risk_flag": risk_flag,
        "options": [
            {
...
def get_recommendations(
    ...
    limit: int = Query(10000, ge=1, le=10000),  # default: return ALL rows
):
    df = get_df()
    if df.empty:
        return []
    ...
    for _, row in filtered.head(limit).iterrows():
        try:
            results.append(_build_item(row.to_dict()))
        except Exception:
            continue
    return results
...
def chat_feed(req: Dict[str, Any]):
    ...
    try:
        ...
        promote_sample = df[df["recommended_action"] == "PROMOTE"][
...
LIVE INVENTORY:
Clearance items (urgent): {clearance_sample}
Promote items (high demand): {promote_sample}
...
Answer the manager with specific product names, store IDs, stock, and rupee (₹) pricing. Never give generic boilerplate."""
        ...
        return {
            "answer": f"AI Assistant error: {e}",
            "response": f"AI Assistant error: {e}",
            "content": f"AI Assistant error: {e}",
        }
